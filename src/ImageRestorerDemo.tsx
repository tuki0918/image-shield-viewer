...
export const ImageRestorerDemo: React.FC = () => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const manifestFile: File | undefined = files.find((f: File) =>
  ...
  const handleSecretSubmit = useCallback(async () => {
    ...
    try {
      ...
      const restoredBlobs = await restorer.restoreImages(
        imageFilesCache,
        manifestCache,
      );
      const urls = restoredBlobs.map((blob) => URL.createObjectURL(blob));
          ...
          <div style={{ marginTop: 16 }}>
            <button
              ...
              onClick={async () => {
                ...
                a.href = url;
                a.download = manifestInfo
                  ? `restored_${manifestInfo.id}.zip`
                  : "images.zip";
                a.click();
